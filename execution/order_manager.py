"""
execution/order_manager.py  —  PolyBot v7.1  v3.0.0
=====================================================
LATEST UPDATE — v3.0.0:
NEW: signal_meta param added to execute() and _record_fill()
     so EV/z_score/confidence/boosts flow through to Telegram alerts.
NEW: alert_order_placed passes full signal_meta dict.
KEPT: triple safety check, FSM, partial fill, _save_state, market_id before pop
"""
...
try:
    from integrations.telegram_formatter import format_trade_open
    _FMT_OK = True
except ImportError:
    _FMT_OK = False
...
class OrderManager:
    ...
    async def execute(
        ...
        async with self.guard._exec_sem:
            ...
            async with self.guard._reg_lock:
                if self.guard.is_duplicate(t_id): return False
                if self.guard.has_open_position(market_id): return False
                try:
                ...
                if not self.client.is_paper_trading():
                    try:
                        ...
                        if pre_balance < size_usd:
                            ...
                            if self.tg: await self.tg.alert_failure(t_id, msg)
                        ...
                        try:
                            ...
                            if existing:
                                ...
                                success = await self._record_fill(
                                    ...
                                    ev=ev, z_score=z_score, strategy=strategy,
                    ...
                    try:
                        ...
                        if ok:
                            ...
                            if resp_filled == 0 and not self.client.is_paper_trading():
                                log.info("AUDIT NOT_FILLED %s — GTC", t_id)
                                success = True
                                if self.tg:
                            ...
                            is_partial = (
                                resp_filled > 0 and resp_original > 0
    ...
    async def _record_fill(
        ...
        log.info("AUDIT FILLED %s %s %s %s $%.2f @ %.4f",
        ...
        if not self.client.is_paper_trading() and pre_balance > 0:
            try:
                ...
                if abs(delta_actual - size_usd) > size_usd * 0.20:
                    msg = f"Balance mismatch: expected ~${size_usd:.2f} actual ~${delta_actual:.2f}"
        ...
        if self.tg:
            if _FMT_OK:
                await self.tg.send(format_trade_open(
                    question=question,
                    side=side,
                    price=price,
                    size_usd=size_usd,
        ...
        try:
            if self.tg:
                await self.tg.alert_partial_fill(
                    t_id, round(filled * price, 2), size_usd, question,
                    timeout_s=self.order_timeout
                )
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    log.warning("partial_monitor fetch %s: %s", t_id, e); continue

                if not order_data:
                    filled = size_usd / max(price, 1e-9); break
                ...
                if self.tg and (time.time() - last_update_t) >= self._partial_update_interval:
                    await self.tg.alert_partial_update(t_id, round(filled*price,2), size_usd, elapsed)
                    last_update_t = time.time()

            else:
                log.warning("PARTIAL TIMEOUT %s — cancelling", t_id)
                try: await self.client.cancel_order(order_id)
                except Exception as e: log.error("partial cancel %s: %s", t_id, e)
                ...
                try: self.guard._fsm_set(t_id, TradeState.CANCELLED)
                ...
                if actual_usd < 1.0:
                    self.guard.set_position_closed(market_id); return

            actual_usd = round(filled * price, 2)
            if actual_usd < 1.0:
                self.guard.set_position_closed(market_id); return

            try: self.guard._fsm_set(t_id, TradeState.FILLED)
            except RuntimeError: pass
            self.guard.record_success(market_id)
            self.guard.set_position_open(market_id)
    ...
    async def _cancel_timed_out_orders(self) -> None:
        ...
        for t_id, order_id in timed_out:
            ...
            if cancelled:
                ...
                try: self.guard._fsm_set(t_id, TradeState.CANCELLED)
                except RuntimeError: self.guard.fail(t_id)
                else:
                    self.guard.active_trades.pop(t_id, None)
                    self.guard._save_state()

                if market_id: self.guard.set_position_closed(market_id)

                if self.tg:
                    await self.tg.alert_cancelled(
                        t_id, order_id, f"timeout_{self.order_timeout:.0f}s", held_sec=held_sec
                    )
            else:
    ...
    async def _alert(self, msg: str) -> None:
        if self.tg:
            try: await self.tg.send(msg)
            except Exception as e: log.warning("_alert failed: %s", e)
